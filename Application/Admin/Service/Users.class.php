...
class Users {
    ...
    function queryUserStatusByUserId($userid){
        $where['userid'] = $userid;
        $uDao = new \UserModel();
        return $uDao->where($where)->find();
    }
    
    function setUserStatusByUserId($userid){
        $where['userid'] = $userid;
        $userCeph = new \UserModel();
        $data['status'] = 0;
        $userCeph->where($where)->save($data);
    }
    
    function RegistUser($umobile, $password, $capacity){
        $ret = array('status'=>7, 'msg'=>'注册用户出现未知错误');
        ...
        if ($query_ret == null || count($query_ret) == 0) {
            ...
            $userid = $query_ret['userid'];
            $user = self::queryUserStatusByUserId($userid);
    
            if ($user['status'] == 2){
                self::setUserStatusByUserId($userid);
                $ret['status'] = 0;
                $ret['msg'] = '手机号码： ['.$umobile.'] 激活成功';
            }else{
                $ret['status'] = 3;
                $ret['msg'] = '注册手机号： ['.$umobile.'] 已经存在!';
            }
        }
        return $ret;
    }
    ...
    function changedPassword($userid, $oldPassword, $newPassword){
        ...
        if (MD5($oldPassword) == $user["password"] || $oldPassword == $user["password"]) {
